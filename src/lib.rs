...
pub async fn download_and_validate_file(
    ...
    match path.validate(hash) {
        Ok(true) => Ok(()),
        Ok(false) => Err(anyhow!(SHAError::FailedValidation(format!("Failed to validate file: {}", path.display())))),
        Err(e) => Err(anyhow!(e)),
    }
}
        ...
        .map(|(idx, item)| {
            ...
            async move {
                ...
                let result = if let Some(sha1) = &item.sha1 {
                    download_and_validate_file(&item.url, &item.path, sha1, Some(internal_tx)).await
                } else {
                    download_file(&item.url, &item.path, Some(internal_tx)).await
                };
